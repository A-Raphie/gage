        ...
        <div className="mx-auto max-w-6xl px-6 py-12">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-mute">
                Every value on this page is read live from the contracts. No
                wallet needed.
              </p>
            </div>
            <button
          ...
          {phase === "ready" && !settlementLive && (
            <div className="mt-10 rounded-[var(--r-panel)] border border-warn/40 bg-surface p-5">
              ...
              <p className="mt-2 text-sm text-mute">
                GageDeal is live on Sepolia and reads below. GageSettlement
                deploys on Creditcoin CC3 the moment the testnet wallet is
                funded; deals will show their Creditcoin side then.
              </p>
            </div>
          )}
          ...
          {phase === "ready" && deals.length === 0 && (
            <div className="mt-10 rounded-[var(--r-panel)] border border-hairline bg-surface p-10 text-center">
              ...
              <p className="mx-auto mt-2 max-w-sm text-sm text-mute">
                The first deal appears here the moment a maker locks a gage and
                an id is registered on Sepolia. Deals are opened from the gage
                CLI today; the console reads and proves them.
              </p>
            </div>
          )}

          {phase === "ready" && deals.length > 0 && (
            <div className="mt-10 overflow-hidden rounded-[var(--r-panel)] border border-hairline">
            ...
            <section className="mt-12">
              ...
              <dl className="mt-6 grid gap-px overflow-hidden rounded-[var(--r-panel)] border border-hairline bg-hairline sm:grid-cols-3">
                {[
                  ...
                  ["ref", selectedDeal.ref?.slice(0, 10) ?? "·"],
                ].map(([k, v]) => (
              ...
              <p className="mt-4 text-xs text-faint">
                ...
                <a
                  className="text-mute underline decoration-hairline-strong underline-offset-4 hover:text-accent"
                  href={`${CC3_EXPLORER}/address/${GAGE_SETTLEMENT_ADDRESS || GAGE_DEAL_ADDRESS}`}
                  rel="noreferrer"
                >
                  {short(GAGE_SETTLEMENT_ADDRESS) || "pending"}
                </a>
              </p>
            </section>
          )}
